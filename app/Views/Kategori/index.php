...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#btnEdit', function(e) {
            ...
            $('[name="title"]').val(dataRow.title);
            $('[name="show"]').val(dataRow.show).trigger('change');
            $('#menu').val(dataRow.menu.map(d => d.id)).trigger('change');
        ...
        grid = $("#datatable").DataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: 'show',
                    render: function(val, type, row, meta) {
                        if(val == '1') return `<span class="badge badge-success text-light">Ya</span>`;
                        else return `<span class="badge badge-warning text-light">Tidak</span>`;
                    }
                },
                {
                    data: 'id',
                    render: function(val, type, row, meta) {
                        return row.menu.map(d => d.label).join(', ');
                    }
                },
                {
                    data: 'id',
        ...
        function buildContent(item) {
            ...
            return `<div class="p-1">
                    <div class="media-body">
                        ...
                        <div>
                            <small class="font-weight-bold"><span class="text-primary">${item.urutan}</span> ${item.title}</small> 
                        </div>
                        <div><b>${item.menu.map(d => d.label).join(', ')}</b></div>
                    </div>
                </div>`;
        }
