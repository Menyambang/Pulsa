<?php
namespace App\Libraries\MyMultipartParser;

use Notihnio\MultipartFormDataParser\MultipartFormDataParser;
use Notihnio\MultipartFormDataParser\MultipartFormDataset;

class MyMultipartFormDataParser extends MultipartFormDataParser
{
    public static function parse() : ?MultipartFormDataset
    {
        ...
        foreach ($bodyParts as $bodyPart) {
            ...
            if (isset($headers['content-disposition']['filename'])) {
                ...
                //parameters
                preg_match('/(.*)\[(.*)\]$/', $headers['content-disposition']['name'], $match);
                if(count($match) == 3){
                    if($match[2] != '')
                        $dataset->params[$match[1]][$match[2]] = $value;
                    else
                        $dataset->params[$match[1]][] = $value;
                    if ($method != "POST" && $method != "GET") {
                        if($match[2] != '')
                            $GLOBALS["_" . $method][$match[1]][$match[2]] = $value;
                        else
                            $GLOBALS["_" . $method][$match[1]][] = $value;
                    }
                }else {
